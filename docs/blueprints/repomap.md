...
- `model.FileRecord`, `ImportRecord`, `TestRecord`, `BlueprintRecord`, `DocRecord`, and
  ...
  fields. Shards use LF line endings, stable sort keys, and a trailing newline.
- `model.IndexMeta` declares schema version, `docs/map`, `index.json`, `repo-map.sqlite`, and all
  shard filenames, including `source_chunks.jsonl`. `IndexMeta.Include` records the normalized
  folder whitelist used to generate the map so `aidlc map --check` can report mismatches between
  saved lock state and generated output.
...
text; `repo-map.sqlite` is a local derived cache, including source chunk FTS rows loaded from JSONL,
and is ignored by `docs/map/.gitignore`. The `.ai/` tree remains static delivery and guidance; it
does not persist repo-specific map state. The root `aidlc.lock.json` owns the saved
...
- Source chunking is local, bounded, deterministic, and lexical. It extracts reviewable code-text
  snippets for map navigation only; it does not use vectors, embeddings, hosted indexes, language
  servers, or remote services.
...
- The integration acceptance gate runs `aidlc map` on the fixture repository, queries through the
  ...
  queries, and asserts mean recall@10 is at least 0.7 across at least 10 labeled queries.
...
- Source chunk coverage proves code files produce bounded deterministic source records and that
  question-shaped queries can retrieve source paths through both public CLI and fallback paths.
